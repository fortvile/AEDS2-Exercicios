...
int PesquisaBinaria(lista *l, int n, char* x)
{   
    //mostrar(l);
    int resp = 1;
    ...
    {
        meio = (esq + dir) / 2;
        //printf("array name[%d]: %s\t %s\n", meio, l->array[meio].name, x );
        if (str_cmp(l->array[meio].name, x) == 0)
        {
            resp = 0;
            esq = l->n;
        }
        else if (str_cmp(l->array[meio].name, x) < 1)
        {
            esq = meio + 1;
        }
        else
...
int main()
{
    ...
    while (!isFim(str2))
    {
        if (PesquisaBinaria(&l, l.n, str2) == 0){
            printf("SIM\n");
        }
        else{
            printf("NAO\n");
