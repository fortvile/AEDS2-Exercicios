...
{
    int i, j;
    int *temp = (int *)malloc(l->n * sizeof(int));
    int *count = (int *)calloc(10, sizeof(int));

    // Primeira passagem: ordena pelo último dígito
    for (i = 0; i < l->n; i++)
    {
        count[getDigit(l->array[i].height, 0)]++;
    }
    for (i = 1; i < 10; i++)
    {
        count[i] += count[i - 1];
    ...
    for (i = l->n - 1; i >= 0; i--)
    {
        temp[count[getDigit(l->array[i].height, 0)] - 1] = l->array[i].height;
        count[getDigit(l->array[i].height, 0)]--;
    }

    // Demais passagens: ordena pelos demais dígitos
    for (j = 1; j < 5; j++)
    {
        mem_cpy(count, (int *)calloc(10, sizeof(int)), 10 * sizeof(int));
        for (i = 0; i < l->n; i++)
        {
            count[getDigit(l->array[i].height, j)]++;
        }
        for (i = 1; i < 10; i++)
        {
            count[i] += count[i - 1];
        ...
        for (i = l->n - 1; i >= 0; i--)
        {
            temp[count[getDigit(l->array[i].height, j)] - 1] = l->array[i].height;
            count[getDigit(l->array[i].height, j)]--;
        }
        mem_cpy(l->array, temp, l->n * sizeof(int));
    }
