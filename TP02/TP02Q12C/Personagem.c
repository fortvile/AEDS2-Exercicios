...
/*void ShellSort(lista *l) {
    ...
    // define o valor do gap inicial como tamanho/2
    gap = tamanho / 2;

    while (gap > 0) {
        // realiza a inserção por comparação com gap
        for (i = gap; i < tamanho; i++) {
            temp = l->array[i];
            j = i;

            while (j >= gap && l->array[j - gap].mass > temp.mass) {
...
void ShellSort(lista *l){
    int h = 1;
    do h = h * 3 + 1; while(h < l-> n);
    do{
        h /= 3;
        for(int i = h + 1; i <= l->n; i++){
            SWCharacter aux = l->array[i]; int j = i;
            while(l->array[j - h].mass > aux.mass){
                l->array[j] = l->array[j - h]; j-=h;
                if( j <= h) break;
            }
            l->array[j] = aux;
        }
