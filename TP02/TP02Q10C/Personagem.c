...
void Selection(lista *l){
    int n = l->n;
    for(int i = 0; i < n - 1; i++){
        int minIndex = i;
        for(int j = i + 1; j < n; j++){
            if(str_cmp(l->array[j].name, l->array[minIndex].name) < 1){
                minIndex = j;
            }
        }
        SWCharacter temp = l->array[minIndex];
        l->array[minIndex] = l->array[i];
        l->array[i] = temp; 
    }
}
...
void SelectionRec(lista *l, int n, int i, int j, int minIndex){
    ...
    if( i < n - 1){
        ...
        if(j < n){
            ...
            //SelectionRec(l, n, i, ++j, minIndex);
        }

        swap(&l->array[i], &l->array[minIndex]);
